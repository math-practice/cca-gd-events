...
.then(text => {
  ...
  const createTimeString = (timeValue) => {
    const timeMatch = timeValue.match(/Date\(\d+,\d+,\d+,(\d+),(\d+),(\d+)\)/);
    return timeMatch ? 
  ...
  json.table.rows.forEach(row => {
    ...
    rowElement.appendChild(createCell(`${dateFormatted}<br>${startTimeFormatted}–${endTimeFormatted}`, 'date'));

    // Name
    const nameContent = row.c[4] && row.c[4].v ? `<a href="${row.c[4].v}">${row.c[3] && row.c[3].v ? row.c[3].v : ''}</a>` : (row.c[3] && row.c[3].v ? row.c[3].v : '');
    rowElement.appendChild(createCell(nameContent, 'name'));

    // Topic
    rowElement.appendChild(createCell(row.c[5] && row.c[5].v ? row.c[5].v : '', 'topic'));

    // Attend
    const attendContent = row.c[7] && row.c[7].v ? `<a href="${row.c[7].v}">${row.c[6] && row.c[6].v ? row.c[6].v : ''}</a>` : (row.c[6] && row.c[6].v ? row.c[6].v : '');
    ...
    rowElement.appendChild(createCell(icon[formatValue] ? icon[formatValue] : '', 'format'));

    // GD Only
    rowElement.appendChild(createCell(row.c[9] && row.c[9].v == "Yes" ? '✔︎' : '', 'gdevent'));

    // Sponsor
    const sponsorContent = row.c[11] && row.c[11].v ? `<a href="mailto:${row.c[11].v}">${row.c[10] && row.c[10].v ? row.c[10].v : ''}</a>` : (row.c[10] && row.c[10].v ? row.c[10].v : '');
